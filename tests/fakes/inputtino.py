...
:class:`FakeInputtinoMouse` records every ``inputtino.Mouse`` call the
production code makes, so tests can assert on the synthesized event
sequence without ever opening ``/dev/uinput``. The fake mirrors the
real method signatures (verified against the installed inputtino
binding) so swapping the class via ``mocker.patch`` is sufficient.
