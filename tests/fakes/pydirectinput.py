...
fake mirrors the function names invoked by
:mod:`vrcpilot.controls.mouse` and :mod:`vrcpilot.controls.keyboard`,
so swapping the module-level symbol via ``mocker.patch.object`` is
sufficient.
"""
...
class FakePyDirectInput:
    ...
    # Production assigns ``FAILSAFE = False`` at import; the slots must
    # exist on the fake or that assignment raises AttributeError.
    FAILSAFE: bool = False
