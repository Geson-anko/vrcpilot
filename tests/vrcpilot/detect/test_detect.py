...
class TestDetectResultDisplayPolygon:
    def test_shifts_each_point_by_screenshot_offset(self):
        shot = _make_screenshot(x=100, y=50)
        det = _make_detection()
        query = _make_query()
        result = DetectResult(screenshot=shot, query=query, detections=(det,))

        assert result.display_polygon(det) == (
            (110.0, 70.0),
            (160.0, 70.0),
            (160.0, 88.0),
            (110.0, 88.0),
    ...
    def test_detection_need_not_be_in_detections(self):
        shot = _make_screenshot(x=100, y=50)
        query = _make_query()
        # Empty detections tuple — display_polygon still works.
...
class TestDetectResultDisplayBBox:
    def test_shifts_xy_keeps_size(self):
        ...
        assert result.display_bbox(det) == (110, 70, 50, 18)
